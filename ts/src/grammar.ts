...
import { Amagama, Rule, RuleSpec, Context, Parser, FuncRef } from './amagama'
...
function grammar(amagama: Amagama) {
  ...
  */
  ...
  // sets key:val on node
  amagama.rule('pair', (rs: RuleSpec, p: Parser) => {
    rs
      .fnref({
        ...fnm,
        '@pair-bc': (r: Rule, ctx: Context) => {
  ...
  // push onto node
  amagama.rule('elem', (rs: RuleSpec, p: Parser) => {
    rs
      .fnref({
        ...fnm,
...
function makeJSON(amagama: any) {
  let justJSON = amagama.make({
    grammar$: false,
    text: { lex: false },
    number: {
      hex: false,
