...
export async function run(argv: string[], console: Console) {
  ...
  if (null != plugins.debug) {
    console.log(amagama.debug.describe() + '\n=== PARSE ===')
  }
...
function handle_plugins(plugins: string[]): Bag {
  ...
  for (let name of plugins) {
    try {
      ...
      // Might be @amagama plugin
      if (!name.startsWith('@')) {
        try {
          out[name] = require('@amagama/' + name)
        } catch (e) {
...
Usage: amagama <args> [<source-text>]*
...
# Output options
> amagama a:b:1 a:c:2 --option JSON.space=2
{
  "a": {
    "b": 1,
    "c": 2
...
# Using plugins (e.g. npm install @amagama/csv)
> amagama -p csv  -o plugin.csv.record.separators=^ "a,b^1,2"
[{"a":"1","b":"2"}]


# Full debug tracing
> amagama -d -o plugin.debug.trace=true a:1
... lots of debug info, including token-by-token trace ...
{"a":1}


See also: http://amagama.senecajs.org
`
