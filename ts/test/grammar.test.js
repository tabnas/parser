...
describe('grammar-options', () => {
  ...
  it('exclude-amagama-imp-no-residual-alts', () => {
  ...
  it('skip-sentinel-exported', () => {
    // SKIP is available on the Amagama object as an immutable symbol.
    assert.equal(typeof Amagama.SKIP, 'symbol')
    assert.equal(Amagama.SKIP, Symbol.for('amagama.SKIP'))
  })


  it('skip-in-deep-merge', () => {
    // SKIP acts like undefined in deep merge — the base value is preserved.
    let { deep } = Amagama.util
    let SKIP = Amagama.SKIP
