...
func TestGrammarCondOpDeclarativeCondition(t *testing.T) {
	...
	mustGrammar(t, j, &GrammarSpec{
		Ref: map[FuncRef]any{
			"@rec": AltAction(func(r *Rule, ctx *Context) {
		...
		Rule: map[string]*GrammarRuleSpec{
			"val": {
				Close: []*GrammarAltSpec{
					{C: map[string]any{"d": CGte(1)}, A: "@rec", G: "custom"},
				},
			},
		},
	})
...
func TestNodeListHelpers(t *testing.T) {
	// ListRef form.
	lr := ListRef{Val: []any{1}}
	out := nodeListAppend(lr, 2).(ListRef)
