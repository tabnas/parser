...
func mr(implicit bool, pairs ...any) tabnas.MapRef {
...
func TestMapRefExplicitOff(t *testing.T) {
	...
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{Map: boolPtr(false)}})
	got, err := j.Parse("{a:1}")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestMapRefCombinedWithTextInfo(t *testing.T) {
	...
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{Map: boolPtr(true), Text: boolPtr(true)}})
	got, err := j.Parse(`{"a":1}`)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
