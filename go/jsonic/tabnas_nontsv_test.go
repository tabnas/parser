...
func TestErrorFormat(t *testing.T) {
	...
	// Pre-split, the package-level Parse used NewParser()'s
	// DefaultLexConfig, which has colour inactive; jsonic.Parse goes
	// through Make(), where colour defaults to active, so disable it
	// explicitly to keep the plain-text format assertions below.
	jq := Make(tabnas.Options{Color: &tabnas.ColorOptions{Active: boolPtr(false)}})
	parse := jq.Parse
...
func TestLexFlagCommentOff(t *testing.T) {
	j := Make(tabnas.Options{Comment: &tabnas.CommentOptions{Lex: boolPtr(false)}})
...
func TestLexFlagNumberOff(t *testing.T) {
	j := Make(tabnas.Options{Number: &tabnas.NumberOptions{Lex: boolPtr(false)}})
...
func TestLexFlagStringOff(t *testing.T) {
	j := Make(tabnas.Options{String: &tabnas.StringOptions{Lex: boolPtr(false)}})
...
func TestLexFlagValueOff(t *testing.T) {
	j := Make(tabnas.Options{Value: &tabnas.ValueOptions{Lex: boolPtr(false)}})
...
func TestLexFlagSpaceOff(t *testing.T) {
	j := Make(tabnas.Options{Space: &tabnas.SpaceOptions{Lex: boolPtr(false)}})
...
func TestLexFlagLineOff(t *testing.T) {
	j := Make(tabnas.Options{Line: &tabnas.LineOptions{Lex: boolPtr(false)}})
