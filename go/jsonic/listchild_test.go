...
func makeChildParser() *tabnas.Tabnas {
	return Make(tabnas.Options{List: &tabnas.ListOptions{Child: boolPtr(true)}})
}
...
func TestListChildList(t *testing.T) {
	...
	expectChild(t, "[:[1,2]]", []any{}, tabnas.ListRef{Val: []any{1.0, 2.0}, Implicit: false})
}

func TestListChildEmptyList(t *testing.T) {
	expectChild(t, "[:[]]", []any{}, tabnas.ListRef{Val: []any{}, Implicit: false})
}

func TestListChildNestedList(t *testing.T) {
	expectChild(t, "[:[[1],[2]]]", []any{},
		tabnas.ListRef{Val: []any{
			tabnas.ListRef{Val: []any{1.0}, Implicit: false},
			tabnas.ListRef{Val: []any{2.0}, Implicit: false},
		}, Implicit: false})
}
...
func TestListChildListElement(t *testing.T) {
	...
	expectChild(t, "[[1,2],:3]", []any{tabnas.ListRef{Val: []any{1.0, 2.0}, Implicit: false}}, 3.0)
}

func TestListChildBeforeListElement(t *testing.T) {
	// [:1,[2,3]] → [ListRef([2,3])] with child=1
	expectChild(t, "[:1,[2,3]]", []any{tabnas.ListRef{Val: []any{2.0, 3.0}, Implicit: false}}, 1.0)
}
...
func TestListChildAutoEnablesListRef(t *testing.T) {
	...
	j := Make(tabnas.Options{List: &tabnas.ListOptions{Child: boolPtr(true)}})
	got, err := j.Parse("[1,2]")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if _, ok := got.(tabnas.ListRef); !ok {
...
func TestListChildWithMapRef(t *testing.T) {
	...
	j := Make(tabnas.Options{
		List: &tabnas.ListOptions{Child: boolPtr(true)},
		Info: &tabnas.InfoOptions{Map: boolPtr(true)},
	})
	got, err := j.Parse("[:{a:1}]")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	child, ok := lr.Child.(tabnas.MapRef)
	if !ok {
...
func TestListChildWithTextInfo(t *testing.T) {
	...
	j := Make(tabnas.Options{
		List: &tabnas.ListOptions{Child: boolPtr(true)},
		Info: &tabnas.InfoOptions{Text: boolPtr(true)},
	})
...
func TestListChildNoExtend(t *testing.T) {
	...
	j := Make(tabnas.Options{
		List: &tabnas.ListOptions{Child: boolPtr(true)},
		Map:  &tabnas.MapOptions{Extend: boolPtr(false)},
	})
