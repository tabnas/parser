...
func fnValueMatcher(lex *tabnas.Lex, _ *tabnas.Rule) *tabnas.Token {
	fwd := lex.Fwd(64)
	...
	src := fwd[:n]
	tkn := lex.Token("#VL", tabnas.TinVL, "ref:"+src[1:], src)
	pnt := lex.Cursor()
	pnt.SI += n
	pnt.CI += n
	return tkn
...
func TestMatchTokenFn(t *testing.T) {
	...
	var idTin tabnas.Tin
	idMatcher := func(lex *tabnas.Lex, _ *tabnas.Rule) *tabnas.Token {
		fwd := lex.Fwd(64)
		n := 0
		for n < len(fwd) && (fwd[n] == '_' ||
	...
	err := j.Grammar(&tabnas.GrammarSpec{
		Options: &tabnas.Options{
			Match: &tabnas.MatchOptions{
				TokenFn: map[string]tabnas.LexMatcher{"#ID": idMatcher},
			},
