...
func TestSafeKey_ConstructorBlockedOnList(t *testing.T) {
	j := Make(tabnas.Options{List: &tabnas.ListOptions{Property: boolPtr(true)}})
...
func TestSafeKey_FalseAllowsProtoOnList(t *testing.T) {
	j := Make(tabnas.Options{
		Safe: &tabnas.SafeOptions{Key: boolPtr(false)},
		List: &tabnas.ListOptions{Property: boolPtr(true)},
	})
