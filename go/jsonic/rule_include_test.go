...
func TestRuleIncludeKeepsOnlyTaggedAlts(t *testing.T) {
	...
	// makeBare: SetOptions re-applies plugins, and the jsonic grammar
	// plugin would reinstall the rule specs, wiping the alts seeded below.
	j := makeBare()
	...
	j.SetOptions(tabnas.Options{Rule: &tabnas.RuleOptions{Include: "keep"}})
	...
	countByTag := func(alts []*tabnas.AltSpec, tag string) int {
		n := 0
		for _, a := range alts {
...
func TestRuleIncludeEmptyIsNoop(t *testing.T) {
	...
	j.SetOptions(tabnas.Options{Rule: &tabnas.RuleOptions{Include: ""}})
...
func TestRuleIncludeMultipleTags(t *testing.T) {
	...
	j.SetOptions(tabnas.Options{Rule: &tabnas.RuleOptions{Include: "alpha,gamma"}})
...
func TestRuleIncludeThenExclude(t *testing.T) {
	...
	j.SetOptions(tabnas.Options{Rule: &tabnas.RuleOptions{Include: "keep", Exclude: "drop"}})
...
func TestGrammarTextAppliesInclude(t *testing.T) {
	...
	// The engine's GrammarText parses its input with the grammar-free
	// tabnas.Make(), which now returns nil — textual grammar declarations
	// silently no-op after the engine/grammar split. Parse the text with
	// the jsonic grammar and feed the options map through the same
	// GrammarSpec.OptionsMap path GrammarText uses.
	parsed, err := Parse(`options: { rule: { include: "keep" } }`)
	if err != nil {
		t.Fatal(err)
	}
	optionsMap := parsed.(map[string]any)["options"].(map[string]any)
	if err := j.Grammar(&tabnas.GrammarSpec{OptionsMap: optionsMap}); err != nil {
		t.Fatal(err)
	}
...
func TestSetOptionsTextInclude(t *testing.T) {
	...
	// setOptionsFromText: the engine's SetOptionsText parses with the
	// grammar-free tabnas.Make() and silently no-ops after the split.
	if _, err := setOptionsFromText(j, `rule: { include: "picked" }`); err != nil {
		t.Fatal(err)
	}
...
func TestRuleIncludeBreaksParsingWhenGrammarHasNoMatchingTags(t *testing.T) {
	...
	j := Make()
	j.SetOptions(tabnas.Options{Rule: &tabnas.RuleOptions{Include: "doesnotexist"}})
