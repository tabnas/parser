...
func TestColorDisabledSuppressesAll(t *testing.T) {
	no := false
	j := Make(tabnas.Options{Color: &tabnas.ColorOptions{Active: &no}})
	_, err := j.Parse(`"unterminated`)
	if err == nil {
		t.Fatal("expected parse error")
	}
...
func TestColorCustomOverrides(t *testing.T) {
	yes := true
	j := Make(tabnas.Options{Color: &tabnas.ColorOptions{
		Active: &yes,
		Reset:  "<R>",
		Hi:     "<HI>",
		Line:   "<LN>",
...
func TestColorToggleViaSetOptions(t *testing.T) {
	...
	j := Make()
	no := false
	j.SetOptions(tabnas.Options{Color: &tabnas.ColorOptions{Active: &no}})
...
func TestColorFromTextDisable(t *testing.T) {
	j, err := setOptionsFromText(Make(), `color: { active: false }`)
	if err != nil {
		t.Fatal(err)
	}
...
func TestColorFromTextCustomCodes(t *testing.T) {
	...
	j, err := setOptionsFromText(Make(), `color: {
		active: true,
		hi: "<HI>",
		lo: "<LO>",
		line: "<LN>",
