...
func runParserTSV(t *testing.T, file string, j *tabnas.Tabnas) {
	t.Helper()
...
func runListChildTSV(t *testing.T, file string, j *tabnas.Tabnas) {
	t.Helper()
	path := filepath.Join(specDir(), file)
	rows, err := loadTSV(path)
	if err != nil {
...
func TestTSVFeatureListPair(t *testing.T) {
	j := Make(tabnas.Options{List: &tabnas.ListOptions{Pair: boolPtr(true)}})
...
func TestTSVFeatureMapChild(t *testing.T) {
	j := Make(tabnas.Options{Map: &tabnas.MapOptions{Child: boolPtr(true)}})
...
func TestTSVFeatureMapChildDeep(t *testing.T) {
	...
	j := Make(tabnas.Options{
		Map:  &tabnas.MapOptions{Child: boolPtr(true)},
...
func TestStripRefsBasic(t *testing.T) {
	lr := tabnas.ListRef{
		Val:      []any{tabnas.MapRef{Val: map[string]any{"a": 1.0}, Implicit: false}, "hello"},
		Implicit: true,
		Child:    tabnas.Text{Str: "world", Quote: `"`},
	}
	got := stripRefs(lr)
