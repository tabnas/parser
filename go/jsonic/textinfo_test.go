...
func expectTextInfo(t *testing.T, input string, expected any) {
	t.Helper()
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{Text: boolPtr(true)}})
...
func tx(quote, str string) tabnas.Text {
	return tabnas.Text{Quote: quote, Str: str}
}
...
func TestTextInfoExplicitOff(t *testing.T) {
	...
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{Text: boolPtr(false)}})
	got, err := j.Parse(`"hello"`)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
