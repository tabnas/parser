...
func lr(implicit bool, vals ...any) tabnas.ListRef {
	if vals == nil {
		vals = []any{}
	}
...
func TestListRefExplicitOff(t *testing.T) {
	...
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{List: boolPtr(false)}})
	got, err := j.Parse("[1,2]")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestListRefCombinedWithTextInfo(t *testing.T) {
	...
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{List: boolPtr(true), Text: boolPtr(true)}})
	...
	for i, expected := range []tabnas.Text{
		{Quote: `"`, Str: "a"},
		{Quote: "'", Str: "b"},
		{Quote: "", Str: "c"},
	} {
		if txt, ok := lr.Val[i].(tabnas.Text); !ok || txt != expected {
