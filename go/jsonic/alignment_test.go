...
func TestAlignmentErrors(t *testing.T) {
	...
	for _, row := range rows {
		...
		je, ok := parseErr.(*tabnas.TabnasError)
		if !ok {
...
func runErrorTSV(t *testing.T, file string, j *tabnas.Tabnas) {
	t.Helper()
...
func TestLexErrorsExcludeTabnasImp(t *testing.T) {
	j := Make(tabnas.Options{Rule: &tabnas.RuleOptions{Exclude: "tabnas,imp"}})
	runErrorTSV(t, "lex-errors.tsv", j)
}

func TestLexErrorsExcludeTabnasImpComma(t *testing.T) {
	j := Make(tabnas.Options{Rule: &tabnas.RuleOptions{Exclude: "tabnas,imp,comma"}})
	runErrorTSV(t, "lex-errors.tsv", j)
}
...
func TestFeatureCommentSuffixLine(t *testing.T) {
	yes := true
	j := Make(tabnas.Options{Comment: &tabnas.CommentOptions{
...
func TestAlignmentMapExtendFalse(t *testing.T) {
	j := Make(tabnas.Options{Map: &tabnas.MapOptions{Extend: boolPtr(false)}})
...
func TestAlignmentMapMergeFunc(t *testing.T) {
	...
	j := Make(tabnas.Options{Map: &tabnas.MapOptions{
		Merge: func(prev, val any, r *tabnas.Rule, ctx *tabnas.Context) any {
...
func TestAlignmentSafeKeyArray(t *testing.T) {
	...
	j2 := Make(tabnas.Options{List: &tabnas.ListOptions{Property: boolPtr(true)}})
...
func TestAlignmentSafeKeyFalse(t *testing.T) {
	j := Make(tabnas.Options{Safe: &tabnas.SafeOptions{Key: boolPtr(false)}})
...
func TestAlignmentStringEscapeErrors(t *testing.T) {
	j := Make(tabnas.Options{String: &tabnas.StringOptions{AllowUnknown: boolPtr(false)}})
...
func TestAlignmentStringAbandon(t *testing.T) {
	j := Make(tabnas.Options{String: &tabnas.StringOptions{Abandon: boolPtr(true)}})
	...
	if err != nil {
		...
		if je, ok := err.(*tabnas.TabnasError); ok && je.Code == "unterminated_string" {
...
func TestAlignmentStringReplace(t *testing.T) {
	j := Make(tabnas.Options{String: &tabnas.StringOptions{
	...
	if got != "aBce" {
		// Empty-string replacements delete the char ('D' -> "").
...
func TestAlignmentNumberExclude(t *testing.T) {
	j := Make(tabnas.Options{Number: &tabnas.NumberOptions{
...
func TestAlignmentLineSingle(t *testing.T) {
	...
	j := Make(tabnas.Options{Line: &tabnas.LineOptions{Single: boolPtr(true)}})
...
func TestAlignmentTextModify(t *testing.T) {
	j := Make(tabnas.Options{Text: &tabnas.TextOptions{
		Modify: []tabnas.ValModifier{
			func(val any) any {
...
func TestAlignmentListPropertyGuard(t *testing.T) {
	j := Make(tabnas.Options{List: &tabnas.ListOptions{
...
func TestAlignmentExclude(t *testing.T) {
	...
	// Re-fetch the rule spec: SetOptions re-applies the jsonic grammar
	// plugin, which rebuilds the RuleSpec objects, so the pre-SetOptions
	// pointer is stale (engine/grammar split).
	valSpec = j.RSM()["val"]
...
func TestAlignmentResultFail(t *testing.T) {
	j := Make(tabnas.Options{Property: &tabnas.PropertyOptions{
		ConfigModify: map[string]tabnas.ConfigModifier{
			"result-fail": func(cfg *tabnas.LexConfig, opts *tabnas.Options) {
...
func TestAlignmentParsePrepare(t *testing.T) {
	...
	j := Make(tabnas.Options{Property: &tabnas.PropertyOptions{
		ConfigModify: map[string]tabnas.ConfigModifier{
			"prepare": func(cfg *tabnas.LexConfig, opts *tabnas.Options) {
				cfg.ParsePrepare = append(cfg.ParsePrepare, func(ctx *tabnas.Context) {
					prepared = true
				})
			},
		},
...
func TestAlignmentEmptyDisabled(t *testing.T) {
	j := Make(tabnas.Options{Lex: &tabnas.LexOptions{Empty: boolPtr(false)}})
...
func TestAlignmentDeepOptions(t *testing.T) {
	...
	base := tabnas.Options{
		...
		Comment: &tabnas.CommentOptions{Lex: boolPtr(true)},
	...
	result := tabnas.Deep(base, over)
	merged, ok := result.(tabnas.Options)
	if !ok {
...
func TestAlignmentDeepOptionsPointer(t *testing.T) {
	...
	result := tabnas.Deep(base, over).(tabnas.Options)
...
func TestAlignmentDeepUndefined(t *testing.T) {
	...
	over := map[string]any{"a": tabnas.Undefined, "b": 3.0}
...
func TestAlignmentModListOrder(t *testing.T) {
	...
	input := []any{"A", "B", "C", "D", "E"}
	result := tabnas.ModList(input, &tabnas.ModListOpts{
		Delete: []int{1},
		Move:   []int{3, 0},
	})
...
func TestAlignmentLexSubscriber(t *testing.T) {
	j := Make()
	var tokens []tabnas.Tin
	j.Sub(func(tk *tabnas.Token, r *tabnas.Rule, ctx *tabnas.Context) {
...
func TestAlignmentFinishRuleFalse(t *testing.T) {
	j := Make(tabnas.Options{Rule: &tabnas.RuleOptions{Finish: boolPtr(false)}})
...
func TestAlignmentSnip(t *testing.T) {
	...
	for _, tt := range tests {
		got := tabnas.Snip(tt.input, tt.maxlen)
		if got != tt.expect {
