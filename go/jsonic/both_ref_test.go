...
func expectBothRef(t *testing.T, input string, expected any) {
	t.Helper()
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{Map: boolPtr(true), List: boolPtr(true)}})
...
func blr(implicit bool, vals ...any) tabnas.ListRef {
	if vals == nil {
		vals = []any{}
	}
	return tabnas.ListRef{Val: vals, Implicit: implicit}
}

// bmr creates a MapRef from key-value pairs.
func bmr(implicit bool, pairs ...any) tabnas.MapRef {
	m := make(map[string]any)
	for i := 0; i+1 < len(pairs); i += 2 {
		k, _ := pairs[i].(string)
		m[k] = pairs[i+1]
	}
	return tabnas.MapRef{Val: m, Implicit: implicit}
}
...
func TestBothRefImplicitListOfImplicitListsInMaps(t *testing.T) {
	...
	outerList, ok := got.(tabnas.ListRef)
	if !ok {
	...
	for i, expectedKey := range []string{"a", "b"} {
		m, ok := outerList.Val[i].(tabnas.MapRef)
		if !ok {
...
func TestBothRefWithTextInfo(t *testing.T) {
	...
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{
		Map:  boolPtr(true),
		List: boolPtr(true),
		Text: boolPtr(true),
	}})
	...
	listVal, ok := m.Val["a"].(tabnas.ListRef)
	if !ok {
	...
	txt, ok := listVal.Val[0].(tabnas.Text)
	if !ok {
...
func TestListRefMetaInitialized(t *testing.T) {
	...
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{List: boolPtr(true)}})
	got, err := j.Parse("[1,2]")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
