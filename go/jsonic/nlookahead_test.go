...
func TestNLookaheadFirstMatchWins(t *testing.T) {
	...
	j.Rule("val", func(rs *tabnas.RuleSpec, _ *tabnas.Parser) {
		rs.Open = append([]*tabnas.AltSpec{
			{
				S: [][]tabnas.Tin{{TA}, {TB}, {TC}},
				A: func(r *tabnas.Rule, ctx *tabnas.Context) { r.Node = "abc" },
			},
			{
				S: [][]tabnas.Tin{{TA}, {TB}, {TD}},
				A: func(r *tabnas.Rule, ctx *tabnas.Context) { r.Node = "abd" },
			},
			{
				S: [][]tabnas.Tin{{TA}, {TB}},
				A: func(r *tabnas.Rule, ctx *tabnas.Context) { r.Node = "ab" },
			},
			{
				S: [][]tabnas.Tin{{TA}},
				A: func(r *tabnas.Rule, ctx *tabnas.Context) { r.Node = "a" },
			},
		}, rs.Open...)
	})
...
func TestNLookaheadBacktrackN3(t *testing.T) {
	...
	j.Rule("collect", func(rs *tabnas.RuleSpec, _ *tabnas.Parser) {
	...
	j.Rule("collectB", func(rs *tabnas.RuleSpec, _ *tabnas.Parser) {
		rs.Open = []*tabnas.AltSpec{{
			S: [][]tabnas.Tin{{TB}},
			A: func(r *tabnas.Rule, ctx *tabnas.Context) {
				seen = append(seen, string(r.O[0].Src))
	...
	j.Rule("collectC", func(rs *tabnas.RuleSpec, _ *tabnas.Parser) {
		rs.Open = []*tabnas.AltSpec{{
			S: [][]tabnas.Tin{{TC}},
			A: func(r *tabnas.Rule, ctx *tabnas.Context) {
				seen = append(seen, string(r.O[0].Src))
