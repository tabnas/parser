...
func TestFixedCheckHookFires(t *testing.T) {
	called := 0
	check := func(lex *tabnas.Lex) *tabnas.LexCheckResult {
		called++
		return nil
	}
	j := Make(tabnas.Options{Fixed: &tabnas.FixedOptions{Check: check}})
...
func TestSpaceCheckHookFires(t *testing.T) {
	called := 0
	j := Make(tabnas.Options{Space: &tabnas.SpaceOptions{Check: func(lex *tabnas.Lex) *tabnas.LexCheckResult {
		called++
		return nil
	}}})
	if _, err := j.Parse("a: 1"); err != nil {
...
func TestLineCheckHookFires(t *testing.T) {
	called := 0
	j := Make(tabnas.Options{Line: &tabnas.LineOptions{Check: func(lex *tabnas.Lex) *tabnas.LexCheckResult {
		called++
		return nil
	}}})
...
func TestTextCheckHookFires(t *testing.T) {
	called := 0
	j := Make(tabnas.Options{Text: &tabnas.TextOptions{Check: func(lex *tabnas.Lex) *tabnas.LexCheckResult {
		called++
		return nil
	}}})
...
func TestNumberCheckHookFires(t *testing.T) {
	called := 0
	j := Make(tabnas.Options{Number: &tabnas.NumberOptions{Check: func(lex *tabnas.Lex) *tabnas.LexCheckResult {
		called++
		return nil
	}}})
...
func TestStringCheckHookFires(t *testing.T) {
	called := 0
	j := Make(tabnas.Options{String: &tabnas.StringOptions{Check: func(lex *tabnas.Lex) *tabnas.LexCheckResult {
		called++
		return nil
	}}})
...
func TestCommentCheckHookFires(t *testing.T) {
	called := 0
	j := Make(tabnas.Options{Comment: &tabnas.CommentOptions{Check: func(lex *tabnas.Lex) *tabnas.LexCheckResult {
		called++
		return nil
	}}})
	if _, err := j.Parse("# hi\na:1"); err != nil {
...
func TestLexCheckCanOverrideResult(t *testing.T) {
	...
	j := Make(tabnas.Options{Fixed: &tabnas.FixedOptions{Check: func(lex *tabnas.Lex) *tabnas.LexCheckResult {
...
func TestInfoMarkerDefault(t *testing.T) {
	yes := true
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{Map: &yes}})
...
func TestInfoMarkerOverride(t *testing.T) {
	yes := true
	j := Make(tabnas.Options{Info: &tabnas.InfoOptions{Map: &yes, Marker: "$meta"}})
...
func TestInfoMarkerFromText(t *testing.T) {
	...
	j, err := setOptionsFromText(Make(), `info: { map: true, marker: "$$info" }`)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParsePrepareHooksRunInOrder(t *testing.T) {
	var order []string
	j := Make(tabnas.Options{Parse: &tabnas.ParseOptions{Prepare: map[string]func(ctx *tabnas.Context){
		"b-second": func(ctx *tabnas.Context) { order = append(order, "second") },
		"a-first":  func(ctx *tabnas.Context) { order = append(order, "first") },
	}}})
...
func TestParsePrepareSkipsNil(t *testing.T) {
	...
	j := Make(tabnas.Options{Parse: &tabnas.ParseOptions{Prepare: map[string]func(ctx *tabnas.Context){
		"a": nil,
		"b": func(ctx *tabnas.Context) { called = true },
	}}})
	if _, err := j.Parse("a:1"); err != nil {
		t.Fatal(err)
	}
...
func TestMatchCheckHookFires(t *testing.T) {
	...
	j := Make(tabnas.Options{Match: &tabnas.MatchOptions{
		Lex:   &yes,
		Check: func(lex *tabnas.Lex) *tabnas.LexCheckResult { called++; return nil },
	}})
	if _, err := j.Parse("a:1"); err != nil {
		t.Fatal(err)
	}
...
func TestErrMsgSuffixRoundTripString(t *testing.T) {
	j, err := setOptionsFromText(Make(), `errmsg: { name: "foo", suffix: "bar" }`)
	if err != nil {
		t.Fatal(err)
	}
	opts := j.Options()
...
func TestErrMsgLinkRoundTrip(t *testing.T) {
	j, err := setOptionsFromText(Make(), `errmsg: { link: "https://example.org/err" }`)
	if err != nil {
		t.Fatal(err)
	}
	opts := j.Options()
...
func TestErrMsgSuffixRoundTripBool(t *testing.T) {
	j, err := setOptionsFromText(Make(), `errmsg: { suffix: false }`)
	if err != nil {
		t.Fatal(err)
	}
	opts := j.Options()
