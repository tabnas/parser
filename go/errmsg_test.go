...
func TestErrMsgLinkInSuffix(t *testing.T) {
	off := false
	j := Make(Options{
		ErrMsg: &ErrMsgOptions{Link: "https://example.org/docs/errors"},
		Color:  &ColorOptions{Active: &off},
	})
	_, err := j.Parse("}")
	if err == nil {
...
func TestErrMsgLinkSuppressedWithSuffixFalse(t *testing.T) {
	j := Make(Options{
		ErrMsg: &ErrMsgOptions{Suffix: false, Link: "https://example.org/x"},
	})
	_, err := j.Parse("}")
	if err == nil {
		t.Fatal("expected error")
...
func TestErrMsgLinkTextForm(t *testing.T) {
	j := Make()
	_, err := j.SetOptionsText(`errmsg: { link: 'https://example.org/text-form' }`)
	if err != nil {
		t.Fatal(err)
	}
	_, perr := j.Parse("}")
...
func TestErrMsgSuffixString(t *testing.T) {
	j := Make(Options{ErrMsg: &ErrMsgOptions{Suffix: "see the manual"}})
	_, err := j.Parse("}")
	if err == nil {
		t.Fatal("expected error")
	}
...
func TestErrMsgSuffixFunc(t *testing.T) {
	j := Make(Options{ErrMsg: &ErrMsgOptions{
...
func TestErrMsgSuffixPlugins(t *testing.T) {
	called := false
	plugin := func(j *Tabnas, opts map[string]any) error {
		called = true
		return nil
	}
	j := Make()
...
func TestErrorFileName(t *testing.T) {
	off := false
	j := Make(Options{Color: &ColorOptions{Active: &off}})
...
func TestErrorNoFileName(t *testing.T) {
	off := false
	_, err := Make(Options{Color: &ColorOptions{Active: &off}}).Parse("}")
	if err == nil {
		t.Fatal("expected error")
	}
...
func TestErrMsgNameHeader(t *testing.T) {
	j := Make(Options{ErrMsg: &ErrMsgOptions{Name: "myapp"}})
	_, err := j.Parse("}")
	if err == nil {
		t.Fatal("expected error")
	}
...
func TestLexErrorCarriesErrMsgConfig(t *testing.T) {
	j := Make(Options{
		ErrMsg: &ErrMsgOptions{Name: "myapp", Link: "https://example.org/lex"},
	})
	_, err := j.Parse(`"unclosed`)
	if err == nil {
		t.Fatal("expected error")
