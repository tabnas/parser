...
> **Line citations.** Every `rs/src/parser.rs:N` citation in this document
> was re-derived against the working tree on 2026-09-22 and verified to name
> the construct the surrounding sentence describes. They had drifted by
> roughly 350-580 lines: the numbers revisions 1-5 were written against match
> no committed state of the file, and twenty perf commits plus the v0.10.0
> `Site` rename have moved everything since. **Bare continuation numbers
> (`:564`, `:2610`, and the like) have NOT been re-derived** unless an
> applied open item touched them; where one names a `parser.rs` line it still
> carries the pre-v0.10.0 numbering and is not authoritative. Each such
> citation quotes or names the construct it means, so re-derive from that
> rather than from the number. Citations into `ts/src/rules.ts` were checked
> and are correct apart from the one this revision fixes (`:535` → `:538`).
...
| id | lens / subject | where it lands | what changed |
|---|---|---|---|
| **U1** | the **before**-open `next` copy moves when the after-phase copy moves | §3.3 (the `:1540` bullet), §10 (the `[S8]` paragraph), §0.2 item 13, §7 (last migration row) | The claim that the before-open `next` argument is "unchanged by this release" is **deleted**. `rs/src/parser.rs:1989` evaluates `let next = is_open.then(\|\| current_rule.snapshot());` **once**, before the binding loop at :1546, so every before-action of the phase sees the rule as it stood before the first of them ran; the lazy replacement takes the copy at the first **bound state action**, so a `bo` `ContextAction` declared before a `bo` state action in `bo_order` now changes what that state action sees in `next`. That is the identical change **[S8]** already records for `ao`/`ac`, and it is a move toward TypeScript (`let next = is_open ? rule : ctx.NORULE`, ts/src/rules.ts:538, passed live at :564). The `AltModifierWithMatch` half of the old sentence is correct and is kept: parser.rs:2586's `then()` sits immediately before the single modifier call at :2092. |
| **U2** | the POP arm must capture the root's result **before** the ruleDone event | §3.3 (the POP arm's `None` branch and its empty-alternatives twin), §6 test 3 | Revision 4 wrote the notify first and `completed_value = Some(current_rule.node.borrow().clone())` second, inverting parser.rs:3174 → :2610 (twin :2698 → :2701). `rule.node` is an `Rc<RefCell<Value>>` that **[S14]** says is still writable through a `&Rule`, so under the inverted order a ruleDone subscriber on the root's final pass could change what `parse()` returns — a parse-result change under §10's freeze. The two statements are swapped back to today's order on both arms. |
| **U3** | the `[T5]` attribute table miscounts `recover_error_pass` | §3.3 (the `[T5]` table) | 11 → **10**, not 11 → 11. Counted from parser.rs:1164-1175: `&self`, `error`, `state`, `alt`, `fallback_error_token`, `src`, `current_rule`, `stack`, `context`, `lexer`, `mode` = 11 today, 10 after `−stack`. The conclusion is unchanged (10 > 7, so the `#[allow]` at :799 stays), but a table whose purpose is to correct a miscount must not contain one. |
...
impl Rule {
    ...
    /// stood before recovery rewrote `current_rule` (parser.rs:1181). It
...
pub fn u_mut(&mut self) -> &mut HashMap<String, Value> {
    ...
    if Rc::strong_count(&self.u) > 1 && !Rc::ptr_eq(&self.u, &empty_values()) {
        // `u` — unlike `n` and `k` (parser.rs:2984-2985/:2500-2501) — is
...
match context.rs.pop() {                                                 // :2598
    ...
    None => {
        ...
        // which is today's order (parser.rs:3174 precedes :2610; the
...
3. `rule_done_sees_the_stack_after_the_transition_and_the_rules_it_moved` (graft 2, matching TS `ctx.rs[0..rsI]` after `ctx.rs[ctx.rsI++] = rule` / `ctx.rs[--ctx.rsI]`, parser.ts:267-285) **[R13, P3]**:
   ...
   - **root's final pass, and the parse-result freeze [U2]**: that same subscriber writes `*rule.node.borrow_mut() = Value::String("tampered".into())`, and the test asserts `parse()` still returns the value the root held when the pass completed. The POP arm's `None` branch reads the result **before** the event (§3.3), as parser.rs:3174 does before :2610, so the write lands on a cell nothing reads again. With the two statements in the other order this assertion fails and §10's "no parse-result change" would be false for any grammar with a ruleDone subscriber.
...
7. `rs/tests/rule_links_test.rs` (graft, Design 2) **[R1, R10, R12, P4]**: the grammar below (fixed tokens `a`–`e`, `x`) exercises push, a nested push, a two-step replace chain and both pops; every expected value is stated **per port**, derived from ts/src/rules.ts:665-666 (push), :692-693 (replace), :713 (pop), :720 (`rule.next`) and go/rule.go:1266-1267/:1292-1293/:1329 — and, before the pin lands, confirmed by running the same grammar once in the TS suite as `ts/test/rule-links.fixture.json` + a case in `ts/test/builtins.test.js` next to `:595` (not `counter.test.js:217`, which only validates path *names*). Rust-after must equal TS on every row; "Rust today" is quoted to show what the test pins.
   ...
   **[T2] Rows evaluated while a child is still running are mandatory, not optional.** A grep of `rs/`, `ts/`, `go/` and `test/spec/*.tsv` finds **no existing case anywhere that uses `parent.next`**, and revision 3's table contained no row where the named rule was still live — so the `Child`-tag regression of **[T1]** was invisible to the entire plan. The `child` open/close rows below are what make it visible: under revision 3's wording they resolve to nothing, under §4's corrected wording they resolve to the live child, and TypeScript and Go resolve to the live child. They must be in `ts/test/rule-links.fixture.json` as well, because the fixture is what establishes the TS column.
   ...
   | | `next.state` | `c` (live) | `c` (live) | `o` (frozen at the open pass, parser.rs:3106 precedes the state change at :3133) — test 8 |
   ...
   | `top` close pass | `child.name` | **`child`** (assigned once at push, :665) | **`child`** (head of the chain) | `child3` (tail, parser.rs:3169) |
   ...
   | | `child.next.name` | `child2` | `child2` (`Chain` successor) | `top` (tail's tag is `Parent`: `top.child_rule` is child3's snapshot, :3169, whose `next_rule` is the top snapshot stored at :3138, resolved by `resolve_snapshot_path`, :4075) |
   | | `child.next.next.name` | `child3` | `child3` | `child` (that top snapshot was taken at :3138, before the pop relinked top at :3165, so it still carries the push-time `child` snapshot in `next_rule`, :2989) |
   | | `child.next.next.next.name` | `top` (`child3.next` = the popped-to parent, :713/:720) | `top` (tail's `Parent` → `Child.parent` = holder) | *(none: the push-time `child` snapshot has `next_rule: None`)* |
   | | `next.name` | **`child`** (`top.next` set at :720 during the push pass, never relinked) | **`child`** (`Child` → `child()`) | `child3` (parser.rs:3170) |
   | | `next.next.name` | `child2` | `child2` | `top` (`top.next_rule` is child3's snapshot, :3170, whose `next_rule` is that same top snapshot) |
   ...
   **The `top` close-pass group is measured, not derived.** Its eight rows were run against this tree by installing the grammar above with the path under test as an ordered set of `c` alternates on `top`'s close, one per candidate rule name, each tagging the node it builds. TypeScript and Go answer `child`, `child2`, `child3`, `top`, `child`, `child2`, `leaf`, `child` in table order; Rust today answers `child3`, `top`, `child`, *(none)*, `child3`, `top`, *(none)*, *(none)*. The three cells revision 5 recorded as *(none)* where a rule in fact resolves — `child.next.name`, `child.next.next.name` and `next.next.name` — are corrected above: the change **[V4]** announces takes those paths from the WRONG rule to the right one, which is stronger than taking them from nothing to something. Go's column needed `NormAlt` repaired first: every plain declarative condition loaded through the serialized door was a silent no-op, so the probe returned the first alternate for all eight paths until that was fixed.
...
| *(unchanged, listed so it is not mistaken for new)* | the `next` argument of an `AltModifierWithMatch` is, and was, an owned copy of the current rule, taken immediately before the single call that consumes it (parser.rs:2586 → :2092, nothing can run in between) — TypeScript passes the live rule at rules.ts:577. Pre-existing, and this PR does not change it **[P6]** |
...
>
> The `next` **argument** of a lifecycle state action is still an owned copy in every case, and this release changes *when* the copy is taken — in the phases that have a copy to take: **`bo`, and `ao`/`ac`** **[S8, U1]**. **`bc` is not one of them.** parser.rs:1989 is `let next = is_open.then(|| current_rule.snapshot());`, inside the single before-phase block that serves both `bo` and `bc` (the `before_enabled` selection is at parser.rs:1957-1961 and the binding selection at :1966-1982), so a before-CLOSE phase runs with `is_open == false` and `next` is `None` before and after this release. Nothing is materialised there, and no assertion on a `bc` state action's `next` can tell `main` from the branch. TypeScript agrees: `let next = is_open ? rule : ctx.NORULE`. It used to be taken once per phase, before the phase's first action (`rule.next_rule.clone()` at parser.rs:601 for the after phases, `is_open.then(|| current_rule.snapshot())` at :1540 for the before ones); it is now taken at the first *bound state action* of the phase. For a pass that pushes or replaces, and for the `next` of a closing pass (the parent), the difference is not observable: the pending rule and the ancestors are reachable only as `&Rule`, so no after-action can change what the copy would have held. Where `next` is the rule itself — an open pass with no route, and **every before-open phase, where `next` is always the rule itself** — it is observable: a `ContextAction`, or a named action that is not a state action, running *before* a state action in `bo`/`ao` order now changes what that state action sees in `next`, where previously the copy predated every action of the phase. The other two move unobservably: `ac`'s `next` is `NextOf::Parent` and `ao`'s on a routing pass is `NextOf::Pending`, neither of which an after-action can reach as `&mut`; and `bc` has no `next` at all. On the fallback ordering (`resolved_action_order`, rs/src/rule.rs:1062-1070) every callback precedes every state action, so a phase that binds both is affected there by construction. TypeScript passes the live rule to both the before handlers (`let next = is_open ? rule : ctx.NORULE`, ts/src/rules.ts:538, used at :564) and the after handlers (:720, used at :728), so it reflects every earlier write; this is a move toward TypeScript in all four phases, not all the way to it, and it is pinned by test 13. Revision 4 recorded this for `ao`/`ac` and asserted the before phases were "unchanged by this release"; one edit causes both, so that assertion is withdrawn **[U1]**. What IS unchanged is the `next` argument of an `AltModifierWithMatch`: it was and remains an owned copy of the current rule, taken immediately before the single call that consumes it (parser.rs:2586 → :2092, with nothing able to run in between), where TypeScript passes the live rule at rules.ts:577 **[P6]**.
>
...
| Design 1: parser.rs:1181 `event_rule = current_rule.clone()` not enumerated; `:583/:616` site-rule clones | Closed: `Rule::detached()` at :817 (tag carried, boxes `None`, documented **[R18]**) and `detached_with_state()` built inside `map_err` at :583/:616 (§2, §3.3), so the debug plugin's per-completion path does not pay a copy. |
...
- `error.rule_stack` byte-identity holds on every arm traced: on a push `ancestors_for` strips the pusher, which is `rs.last()` after the push exactly as `stack.last()` is today; on a pop today's `stack` at notify time is **already** popped at parser.rs:3165, so the post-transition `ancestors()` matches.
